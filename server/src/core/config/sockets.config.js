...
import User from "../../modules/funcionarios/funcionarios.model.js";

const secretKey = process.env.JWT_SECRET;

export const userSockets = new Map();

export function addSocketForUser(userId, socket) {
  const key = String(userId);
  if (!userSockets.has(key)) userSockets.set(key, new Set());
  userSockets.get(key).add(socket.id);
}

export function removeSocketForUser(userId, socket) {
  const key = String(userId);
...
export function forceLogoutUser(io, userId, reason = "estado_false") {
  const key = String(userId);
...
export function startChangeStream(io) {
...
export function startPolling(io, intervalMs = 15000) {
  setInterval(async () => {
    try {
...
export function broadcastContenidoUpdate(io) {
  console.log(
...
export function notifySolicitudUpdateToUser(io, userId, solicitudData) {
  const key = String(userId);
  const sockets = userSockets.get(key);
  if (!sockets) {
    console.log(
...
export function notifySolicitudUpdatedToApprover(io, userId, solicitudData) {
  const key = String(userId);
  const sockets = userSockets.get(key);
  if (!sockets) {
    console.log(
